...
function BaseCombobox(props: BaseProps) {
	...
	const internalValue = value == null ? '' : String(value);
								...
								renderDisplay ? (
									...
									<span className='block truncate'>
										{typeof selected.label === 'string'
											? selected.label
											: selected /* as any */.searchText ?? selected.label}
									</span>
								)
							) : (
								placeholder
								...
								{options.map((opt) => {
									const searchSource =
										typeof opt.label === 'string'
											? opt.label
											: opt.searchText ?? opt.value;
									const normalizedLabel = `${searchSource ?? ''}`
										.toString()
										.trim()
										.toLowerCase();
									const normalizedValue = String(opt.value)
										...
										.trim()
										.toLowerCase();
									const candidateValue = [normalizedLabel, normalizedValue]
										.filter(Boolean)
										.join(' ');
									...
									return (
										...
										>
											<Check
												className={cn(
													'mr-2 size-4',
													String(opt.value) === internalValue
														? 'opacity-100'
														: 'opacity-0'
												)}
											/>
