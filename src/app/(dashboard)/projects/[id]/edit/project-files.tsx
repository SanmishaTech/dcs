...
export function ProjectFiles({ projectId }: { projectId: number }) {
  ...
  async function handleUploadSubmit(values: { file: File | null; title: string }) {
    if (!values.file) { toast.error('Select a file'); return; }
            ...
            <form onSubmit={form.handleSubmit(handleUploadSubmit)} className='flex flex-col gap-2 w-full max-w-xl'>
              <div className='flex flex-wrap gap-3 items-end'>
                ...
                <div className='flex items-center pb-2'>
                  <AppButton size='sm' type='submit' disabled={uploading || !titleValid || !fileValue}>{uploading ? 'Uploading...' : 'Upload'}</AppButton>
                </div>
              </div>
            </form>
          </Form>
