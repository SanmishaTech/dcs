...
export async function GET(req: NextRequest) {
  ...
  const sortableFields = new Set(["name", "email", "role", "status", "createdAt", "lastLogin"]);
...
export async function POST(req: NextRequest) {
  ...
  const { name, email, password, role = 'user', status = true } = (body as Partial<{ name: string; email: string; password: string; role: string; status: boolean }>) || {};
  ...
  try {
    ...
    const created = await prisma.user.create({
      data: { name: name || null, email, passwordHash, role, status: Boolean(status) },
      select: { id: true, name: true, email: true, role: true, status: true, lastLogin: true, createdAt: true }
    });
...
export async function PATCH(req: NextRequest) {
  ...
  let body: unknown;
  try { body = await req.json(); } catch { return Error('Invalid JSON body', 400); }
  const { id, status, role, name } = (body as Partial<{ id: number | string; status?: boolean; role?: string; name?: string }>) || {};
