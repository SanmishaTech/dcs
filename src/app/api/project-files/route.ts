...
export async function GET(req: NextRequest) {
  ...
  select: { id: true, originalName: true, title: true, filename: true, mimeType: true, size: true, createdAt: true, uploadedById: true },
  });
...
export async function POST(req: NextRequest) {
  ...
  if (!ALLOWED_MIME_PREFIXES.some(p => mimeType?.startsWith(p))) return Error('File type not allowed', 415);
  ...
  const uploadRoot = path.join(process.cwd(), 'upload', 'projects', String(pid));
  ...
  const { projectId, originalName: on, mimeType: mt, size: sz, filename, title: t } = (body as Partial<{ projectId: number|string; originalName: string; mimeType: string; size: number; filename: string; title?: string }>);
    if (projectId == null) return Error('projectId required', 400);
    pid = Number(projectId); if (Number.isNaN(pid)) return Error('Invalid projectId', 400);
  if (!on || !mt || !filename || typeof sz !== 'number' || !t || !t.trim()) return Error('Missing file metadata', 400);
  originalName = on; mimeType = mt; size = sz; storedFilename = filename; title = t;
  }

  // Basic presence check after branching
  if (!originalName || !mimeType || !storedFilename || typeof size !== 'number') {
  ...
  select: { id: true, projectId: true, originalName: true, title: true, mimeType: true, size: true, filename: true, createdAt: true },
    });
...
export async function DELETE(req: NextRequest) {
  ...
  const fid = Number(id); if (Number.isNaN(fid)) return Error('Invalid id', 400);

  try {
    // Get filename first for disk cleanup
    const record = await prisma.projectFile.findUnique({ where: { id: fid }, select: { id: true, filename: true, projectId: true } });
    if (!record) return Error('File not found', 404);
    const deleted = await prisma.projectFile.delete({ where: { id: fid }, select: { id: true, projectId: true } });
    // Attempt unlink (ignore failure)
    const uploadRoot = path.join(process.cwd(), 'upload', 'projects', String(record.projectId));
    const legacyRoot = path.join(process.cwd(), 'upload', String(record.projectId)); // backward compatibility cleanup
