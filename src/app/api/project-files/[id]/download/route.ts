...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { user } = auth;
  const { id } = await ctx.params; const fid = Number(id);
  if (Number.isNaN(fid)) return Error('Invalid id', 400);

  try {
    const fileRec = await prisma.projectFile.findUnique({
      where: { id: fid },
      select: { id: true, projectId: true, filename: true, originalName: true, mimeType: true, size: true },
    });
    if (!fileRec) return Error('File not found', 404);
    if (user.role === 'project_user') {
      const membership = await prisma.projectUser.findFirst({ where: { projectId: fileRec.projectId, userId: user.id }, select: { id: true } });
  ...
  const legacy = path.join(process.cwd(), 'upload', String(fileRec.projectId), fileRec.filename);
    ...
    let data: Buffer; try { data = await fs.readFile(diskPath); } catch { return Error('Failed to read file', 500); }
