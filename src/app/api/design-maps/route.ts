...
// Local select for crack fields used by design-maps API
...
export async function GET(req: NextRequest) {
	...
	const where: { projectId: number; crackIdentificationId?: number } = {
		projectId,
	};
	if (crackIdentificationId)
		where.crackIdentificationId = crackIdentificationId;
	try {
		const maps = await prisma.designMap.findMany({
			where,
			orderBy: { id: 'asc' },
			include: {
				crackIdentification: { select: CRACK_FOR_DISPLAY_SELECT },
			},
		});
		return Success({ items: maps });
	} catch {
		return Error('Failed to fetch design maps');
	}
}
...
export async function POST(req: NextRequest) {
	...
	const { projectId, crackIdentificationId, x, y, width, height } =
		body as Partial<{
			projectId: number;
	...
	if (!projectId) return Error('projectId required', 400);
	if (!crackIdentificationId)
		return Error('crackIdentificationId required', 400);
	const coords = [x, y, width, height];
	if (
		coords.some((v) => v === null || v === undefined || typeof v !== 'number')
	)
		return Error('x,y,width,height required', 400);
	try {
		// Ensure crackIdentification belongs to project
		const crack = await prisma.crackIdentification.findUnique({
			where: { id: crackIdentificationId },
			select: { projectId: true },
		});
		if (!crack || crack.projectId !== projectId)
			return Error('Crack not found in project', 404);
		const created = await prisma.designMap.create({
			data: {
				projectId,
		...
		if (err.code === 'P2002')
			return Error('Design map already exists for crack', 409);
