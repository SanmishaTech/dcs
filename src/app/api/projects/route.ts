...
export async function GET(req: NextRequest) {
	...
	try {
		...
		if (ids.length > 0) {
			...
			const sizeByProject = new Map<number, number>(
				filesGrouped.map((g) => [g.projectId, Number(g._sum.size || 0)])
			);
			...
			for (const g of videosGrouped as Array<{
				projectId: number;
...
export async function POST(req: NextRequest) {
	...
	if (contentType.includes('multipart/form-data')) {
		...
		if (isFileLike(file)) {
			if (!file.type?.startsWith('image/'))
				return Error('Design image must be an image', 415);
			if (file.size > 20 * 1024 * 1024)
				return Error('Design image too large (max 20MB)', 413);
			try {
				const buf = Buffer.from(await file.arrayBuffer());
				designImageFile = { buffer: buf, originalName: file.name };
			} catch {
				return Error('Failed to read design image', 500);
			}
		}
	} else {
		let body: unknown;
	...
	try {
		...
		if (designImageFile) {
			...
			try {
				await fs.writeFile(path.join(dir, filename), designImageFile.buffer);
			} catch {
				return Error('Failed to store design image', 500);
			}
			await prisma.project.update({
				where: { id: created.id },
				data: { designImage: filename },
			});
